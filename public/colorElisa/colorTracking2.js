...
$(document).ready(function() {
    ...
    var dataToPlot_HSV = new Array(1);
    var dataToPlotLabel_HSV = new Array(1);

    for (var j = 0; j < numAreas; j++) {
        dataToPlot[j] = new Array();
        dataToPlotLabel[j] = "";
        dataToPlot_HSV[j] = new Array();
        dataToPlotLabel_HSV[j] = "";
    }
    var incomingData;
    ...
    divImage.onload = function() {
        ...
        for (var i = 0; i < numAreas; i++) {
            ...
            if (dataToPlot[i].length > totalPoints) { //dataToPlot[i].push([now, nextA ]);
                ...
                dataToPlot_HSV[i] = dataToPlot_HSV[i].splice(1, totalPoints);
            }
        }
        countPoints++;
        ...
        incomingData = [{
            ...
            data: dataToPlot_HSV[0],
            label: dataToPlotLabel_HSV[0]
        }, {
            data: dataToPlot_HSV[1],
            label: dataToPlotLabel_HSV[1]
        }, {
            data: dataToPlot_HSV[2],
            label: dataToPlotLabel_HSV[2]
        }, {
            data: dataToPlot_HSV[3],
            label: dataToPlotLabel_HSV[3]
        }, {
            data: dataToPlot_HSV[4],
            label: dataToPlotLabel_HSV[4]
        }, {
            data: [0],
            yaxis: 2,
        }, ];
        ...
        plot = $.plot("#flot1", incomingData, {
            ...
            colors: ["#fff", "#ccc", "#888", "#444", "#000","#f00","#c00","#800","#400","#300"],
            xaxes: [{
                //mode: "time",
                //timeformat: "%H:%M:%S",
                //timezone: "browser",
